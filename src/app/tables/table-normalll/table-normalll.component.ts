...
@Component({
  selector: 'app-table-normalll',
  templateUrl: './table-normalll.component.html',
  styleUrls: ['./table-normalll.component.css']
})
export class TableNormalllComponent implements OnInit {
...
data = [
  ...
  this.uploadService.salesUsersList().subscribe(
         (res)=>{
             console.log(res)
             this.data = res['data'];
    }  );
...
getMonth(month){
  this.uploadService.s2(month).subscribe(
         (res)=>{
             console.log(res)
             this.data = res['data'];
    }  );
...
onCustomAction(event) {
  ...
  this.router.navigate(['/admin/pages/page-faqq', event.data.cutomer_id, month]);
